...
def adddata():  # block for adding data
    ...
    mycur.execute(sql_5)
    db.commit()
    print("\nYOUR ACCOUNT REGISTERATION IS COMPLETE!!!!!!")
...
def new_tea():
    ...
    A = 1
    while A > 0:
        TEACHER_SUBJ = (input("ENTER SUBJECT 1:")).upper()[0]
        A = 0

    A = 1
    while A > 0:
        TEACHER_SUBJ2 = int(input("ENTER SUBJECT 2(Enter nil if none):"))
        A = 0
    ...
    while C>0:
        ...
        for x in h:
            ...
            if A==1:
                ...
                while B>0:#password check for fetch data block
                        password=input("ENTER PASSWORD:")
                        sql="SELECT TEACHER_PASSWORD,TEACHER_ID FROM TEACHERS WHERE TEACHER_ID=teacher_id"
                        mycur.execute(sql)
                        h=mycur.fetchall()
                        for x in h:
                            if x[1]==teacher_id:
